...
class Perfil  extends Controller {
    ...
    public function valpass() {
        ...
        if (!empty($errores)) {
            ...
            # obtenemos los datos modificados del usuario
            $userModify = $this->model->getUserId($_SESSION['id']);
            try {
                $mail = new PHPMailer(true);
                $mail->CharSet = "UTF-8";
                $mail->Encoding = "quoted-printable";
                ...
                //Cabecera del email
                $remitente = USER_MAIL;
                $destinatario = $userModify->email;

                $mail->setFrom($remitente, 'Gesbank S.L');
                $mail->addAddress($destinatario,$userModify->name);
                $mail->addReplyTo($remitente, 'Gesbank S.L');
    ...
    public function delete() {
        ...
        if (!isset($_SESSION['id'])) {
            ...
            header("location:".URL. "login");
             ...
             $userDel= $this->model->getUserId($_SESSION['id']);
             try {
                 $mail = new PHPMailer(true);
                 $mail->CharSet = "UTF-8";
                 $mail->Encoding = "quoted-printable";
                 ...
                 //Cabecera del email
                 $remitente = USER_MAIL;
                 $destinatario = $userDel->email;
 
                 $mail->setFrom($remitente, 'Gesbank S.L');
                 $mail->addAddress($destinatario,$userDel->name);
                 $mail->addReplyTo($remitente, 'Gesbank S.L');
