    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link <?=(in_array($_SESSION['id_rol'],$GLOBALS['alumno']['new']))?'activate':'disabled'?>" href="<?= URL ?>alumno/new">Nuevo</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle 
                    <?=(in_array($_SESSION['id_rol'],$GLOBALS['alumno']['order']))?'activate':'disabled'?>" href="#" role="button" data-bs-toggle="dropdown"
                        aria-expanded="false">
                        Ordenar
                    </a>
            ...
            <form class="d-flex" role="search" method="GET" action="<?= URL ?>alumno/filter">
                ...
                <?=(!in_array($_SESSION['id_rol'],$GLOBALS['alumno']['filter']))?'disabled':null?>" type="submit">Buscar</button>
            </form>
        </div>
    </div>
</nav>